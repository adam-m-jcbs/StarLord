...
contains
  ...
  subroutine ca_mol_single_stage(time, &
    ...
    use advection_util_module, only: ht, allocate_ht, deallocate_ht, ut
#ifdef CUDA
    ...
    integer,  target, intent(in   ) :: uin_lo(3), uin_hi(3)
    integer,  target, intent(in   ) :: uout_lo(3), uout_hi(3)
    integer,  target, intent(in   ) :: q_lo(3), q_hi(3)
    integer,  target, intent(in   ) :: qa_lo(3), qa_hi(3)
    integer,  target, intent(in   ) :: updt_lo(3), updt_hi(3)
    integer,  target, intent(in   ) :: flux1_lo(3), flux1_hi(3)
    integer,  target, intent(in   ) :: flux2_lo(3), flux2_hi(3)
    integer,  target, intent(in   ) :: flux3_lo(3), flux3_hi(3)
    integer,  target, intent(in   ) :: area1_lo(3), area1_hi(3)
    integer,  target, intent(in   ) :: area2_lo(3), area2_hi(3)
    integer,  target, intent(in   ) :: area3_lo(3), area3_hi(3)
    integer,  target, intent(in   ) :: vol_lo(3), vol_hi(3)

    real(rt), target, intent(in   ) :: uin(uin_lo(1):uin_hi(1), uin_lo(2):uin_hi(2), uin_lo(3):uin_hi(3), NVAR)
    real(rt), target, intent(inout) :: uout(uout_lo(1):uout_hi(1), uout_lo(2):uout_hi(2), uout_lo(3):uout_hi(3), NVAR)
    real(rt), target, intent(inout) :: q(q_lo(1):q_hi(1), q_lo(2):q_hi(2), q_lo(3):q_hi(3), NQ)
    real(rt), target, intent(inout) :: qaux(qa_lo(1):qa_hi(1), qa_lo(2):qa_hi(2), qa_lo(3):qa_hi(3), NQAUX)
    real(rt), target, intent(inout) :: update(updt_lo(1):updt_hi(1), updt_lo(2):updt_hi(2), updt_lo(3):updt_hi(3), NVAR)
    real(rt), target, intent(inout) :: flux1(flux1_lo(1):flux1_hi(1), flux1_lo(2):flux1_hi(2), flux1_lo(3):flux1_hi(3), NVAR)
    real(rt), target, intent(inout) :: flux2(flux2_lo(1):flux2_hi(1), flux2_lo(2):flux2_hi(2), flux2_lo(3):flux2_hi(3), NVAR)
    real(rt), target, intent(inout) :: flux3(flux3_lo(1):flux3_hi(1), flux3_lo(2):flux3_hi(2), flux3_lo(3):flux3_hi(3), NVAR)
    real(rt), target, intent(in   ) :: area1(area1_lo(1):area1_hi(1), area1_lo(2):area1_hi(2), area1_lo(3):area1_hi(3))
    real(rt), target, intent(in   ) :: area2(area2_lo(1):area2_hi(1), area2_lo(2):area2_hi(2), area2_lo(3):area2_hi(3))
    real(rt), target, intent(in   ) :: area3(area3_lo(1):area3_hi(1), area3_lo(2):area3_hi(2), area3_lo(3):area3_hi(3))
    real(rt), target, intent(in   ) :: vol(vol_lo(1):vol_hi(1), vol_lo(2):vol_hi(2), vol_lo(3):vol_hi(3))
